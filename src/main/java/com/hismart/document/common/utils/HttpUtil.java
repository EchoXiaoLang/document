...
public class HttpUtil {
    ...
    private static final ResourceBundle bundle = ResourceBundle.getBundle("httpClient");
    private static final String maxTotal = bundle.getString("maxTotal");
    private static final String maxPerRoute = bundle.getString("maxPerRoute");
    private static final String connectionTimeout = bundle.getString("connectionTimeout");
    private static final String waitTimeout = bundle.getString("waitTimeout");
    private static final String readTimeout = bundle.getString("readTimeout");
    private static final String keepAliveTime = bundle.getString("keepAliveTime");
    ...
    private static synchronized void initPools() {
        if (client == null) {
            ...
            RequestConfig requestConfig = RequestConfig.custom()
                    .setConnectTimeout(Integer.valueOf(connectionTimeout) * 1000).setConnectionRequestTimeout(Integer.valueOf(waitTimeout) * 1000)
                    .setSocketTimeout(Integer.valueOf(readTimeout) * 1000).build();

            client = HttpClients.custom().setConnectionManager(connectionManager).setKeepAliveStrategy(defaultStrategy).setDefaultRequestConfig(requestConfig)
    ...
    private static ConnectionKeepAliveStrategy defaultStrategy = (response, context) -> {
        int keepTime = Integer.valueOf(keepAliveTime);
